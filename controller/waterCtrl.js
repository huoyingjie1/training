...
const device5 = iot.device({
    productKey: 'a1P5LOZdBnU', //将<productKey>修改为实际产品的ProductKey
    deviceName: 'water',//将<deviceName>修改为实际设备的DeviceName
    deviceSecret: '87f61d7f3de38b8ecb66b78ab08314cf',//将<deviceSecret>修改为实际设备的DeviceSecret
});

const ledSubTopic = 'a1P5LOZdBnU/water/user/get'
//监听connect事件
device5.on('connect', () => {
    //将<productKey> <deviceName>修改为实际值
    device5.subscribe('/a1P5LOZdBnU/water/user/get'); //subscribe从阿里云收到东西
    console.log('喷水器连接成功!');
    device5.publish('/a1P5LOZdBnU/water/user/update', 'hello world!');//publish发消息给谁
});
...
device5.onProps((cmd) => {
    ...
    for (var key in cmd.params) {
        if (key == 'WorkSwitch') { //判断是否设置的是LightSwitch属性
            console.log('喷水器的状态 ', key);
            //示例代码将云端设置的属性在本地进行保存，实际产品开发时需要修改为去将灯打开或者关闭
            WorkSwitch = cmd.params.WorkSwitch;
            waterStatus = Number(cmd.params.WorkSwitch);
            //本地设置完毕之后，将更新后的状态报告给云端。
            //注意：云端下发命令后，云端属性的值并不会改变，云端需要等待来自设备端的属性上报
            if (WorkSwitch == 0) {
                console.log('喷水器从云端关闭');
            }
            else {
                console.log('喷水器从云端打开');
            }
            device5.postProps({ 'WorkSwitch': WorkSwitch });
        }
    }
})
...
module.exports = {
    report: function (req, resp) {
        ...
        const water_status = req.params['water_status'];

        device5.postProps({
            WorkSwitch: Number(water_status)
        }, (res) => {
            // console.log(res); 
        });
        // 打印id和状态
        ...
        const obj = {
            id: id,
            success: true, // 是否成功 
            status: waterStatus, // 将云服务器的设备状态放入status字段里
            fire:firestatus,
        };
        // 发送给PT 
    ...
    update: function (req, resp) {
        const obj = req.body;
        const items = {
            WorkSwitch: obj.status
        };
        var params = {
            "RegionId": "cn-shanghai",
            "Items": JSON.stringify(items),
            "ProductKey": "a1P5LOZdBnU",
            "DeviceName": "water"
        }
        var requestOption = {
            method: 'POST'
        ...
        client.request('QueryDevicePropertyStatus', params, requestOption).then((result) => {
            if (result.Success) {
                for (let statusInfo of result.Data.List.PropertyStatusInfo) {
                    if (statusInfo.Identifier === 'WorkSwitch') {
                        WorkSwitch = Number(statusInfo.Value);
                        console.log(WorkSwitch);
                        // 这里设置获得的属性值 
                    }
                }
            }
